...
func encodeSuites(buf []byte, suites []CipherSuite) ([]byte, error) {
	...
	arr := make([]cbor.Value, len(suites))
	for i, s := range suites {
		arr[i] = cbor.Uint(uint64(s))
	}
	return cborEncodeValue(buf, cbor.MakeArrayFromSlice(arr))
}
...
func encodeCBORArray(items ...cbor.Value) ([]byte, error) {
	return cborEncodeValue(nil, cbor.MakeArrayFromSlice(items))
}
